...
def collapse_small_triangles(points, edges_full_or_uv, threshold=5.0, marker_new=9):
    """
    Identifies 3-cliques (triangles) where all edges are <= threshold,
    and collapses them into a single vertex at their centroid.
    """
    P = np.asarray(points, dtype=float)
    E = np.asarray(edges_full_or_uv)
    has_marker = (E.ndim == 2 and E.shape[1] >= 3)
    ...
    # 1. Find all triangles with small edges
    small_triangles = []
    nodes = sorted(adj.keys())
    for i in range(len(nodes)):
        u = nodes[i]
        nbrs = adj[u]
        for idx_v in range(len(nbrs)):
            v = nbrs[idx_v]
            if v <= u: continue
            
            # length u-v
            d_uv = np.linalg.norm(P[u] - P[v])
            if d_uv > threshold: continue
    ...
    # 2. Group vertices using Union-Find
    ...
    # 3. Collapse groups to centroids
    ...
    for root, members in groups.items():
        if len(members) > 1:
            centroid = np.mean(P[members], axis=0)
    ...
    # 4. Remap edges and remove self-loops/duplicates
    new_edges_uv = []
    for u, v in uv:
    ...
    if new_edges_uv:
        uv_final = np.unique(new_edges_uv, axis=0)
    else:
        uv_final = np.empty((0, 2), dtype=int)
    ...
    if has_marker:
        ...
        for a, b in uv_final:
            ...
            m = orig_map.get(key, int(marker_new))
            full.append([int(a), int(b), int(m)])
        return P2, np.asarray(full, dtype=int)
