...
def compute_persistence_swr(input_filename, output_dir):
    ...
    bin_path = os.path.join(base_dir, "code", "persistence_swr", "spt_cpp")
    command = bin_path + " " + input_filename + ' ' + output_dir + ' 0 1'
    os.system(command)
...
def compute_graph_reconstruction(sorted_weights_filename, edge_persistence_filename, persistence_threshold, output_dir):
    ...
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    bin_path = os.path.join(base_dir, "code", "phat_morse_src", "a.out")
    command = bin_path + " " + sorted_weights_filename + ' ' + edge_persistence_filename + ' ' + str(persistence_threshold) + ' ' + output_dir
    os.system(command)
