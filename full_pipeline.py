...
# =========================================================
# PHASE 1: SKELETONIZATION & EXTRAPOLATION FUNCTIONS
# =========================================================
...
def run_reconstruction(points, output_dir, args, prefix=""):
    ...
    dm.pcd.build_sparse_weighted_rips_filtration(feature_filename, output_dir, args.k, args.metric, args.epsilon)
    ...
    dm.pcd.compute_graph_reconstruction(sorted_weights_filename, edge_filename, args.persistence_threshold, output_dir)
    ...
    added_back = MomentumConnect(filtered_edges, bad2, points_sorted, args.keep_tau, 30) 
...
def parse_args():
    ...
    parser.add_argument("--voxel-size", type=float, default=10, help="Voxel size for reconnection scale")
...
def main():
    ...
    pcd_path = os.path.join(data_dir, args.pcd_name)
    
    print("\n--- INITIALIZATION ---")
    print(f"Loading raw PCD from {pcd_path}...")
    pcd_obj = o3d.io.read_point_cloud(pcd_path)
    raw_points = np.asarray(pcd_obj.points)

    # 1. PREPROCESSING (Replaces simple downsampling)
    print("\n--- PHASE 1: SKELETONIZATION AND EXTRAPOLATION ---")
    skeletonized_points = perform_advanced_skeletonization(raw_points)

    # 2. INNER/OUTER SPLIT
    print("\n--- PHASE 2: GRAPH RECONSTRUCTION ---")
    print("Splitting skeletonized PCD into Inner and Outer regions with 2% overlap...")
    P_in, P_out = split_cloud_with_overlap(skeletonized_points, inner_ratio=0.8, overlap_ratio=0.02)
    print(f"Inner points: {len(P_in)}, Outer points: {len(P_out)}")

    print("Running Pipeline on Inner Region...")
    P_in_sorted, E_in = run_reconstruction(P_in, f"results/{args.dataset}-inner/", args, prefix="INNER")

    print("Running Pipeline on Outer Region...")
    P_out_sorted, E_out = run_reconstruction(P_out, f"results/{args.dataset}-outer/", args, prefix="OUTER")

    print("Merging Graphs...")
    P_comb, E_comb = merge_graphs(P_in_sorted, E_in, P_out_sorted, E_out)

    print("Snapping overlap boundaries together...")
    E_comb = handshake_join(P_comb, E_comb, max_snap_distance=args.voxel_size * 1.5)

    # 3. GRAPH RECONNECTION
    print("\n--- PHASE 3: ADVANCED RECONNECTION ---")
    
    MAX_REACH = args.voxel_size * 3.0 
    print(f"Using conservative max reach of {MAX_REACH} for reconnections...")
    ...
    P_latched, E_latched = beam_latch_from_degree1(
        P_rays, E_pruned2, beam_radius=args.voxel_size * 2.0, max_length=MAX_REACH, pick="forward"
    )
    ...
    # 3d. Collapse micro-triangles to clean up artifacts without deforming lines
    print("Collapsing small triangles...")
    P_collapsed, E_collapsed = collapse_small_triangles(
        P_conn, E_conn, threshold=args.voxel_size * 20 
    )

    # 3e. Final Clean
    print("Cleaning up final isolated points...")
    P_final, E_final = remove_isolated_points(P_collapsed, E_collapsed)
    print(f"Final edge count: {E_final.shape[0]}")

    # =========================================================
    # SAVE AND VISUALIZE
    # =========================================================
    os.makedirs(f"results/{args.dataset}/", exist_ok=True)
    np.savetxt(f"results/{args.dataset}/edge_detour_filtered.txt", E_final, fmt="%d")
    np.savetxt(f"results/{args.dataset}/sorted_features.txt", P_final, fmt="%.6f")

    if not args.no_vis:
        print("Visualizing Final Stitched Graph...")
        pcd_vis = o3d.geometry.PointCloud()
        pcd_vis.points = o3d.utility.Vector3dVector(P_final)
        pcd_vis.paint_uniform_color([0.1, 0.7, 0.9])

        line_set = o3d.geometry.LineSet()
        line_set.points = o3d.utility.Vector3dVector(P_final)
        line_set.lines = o3d.utility.Vector2iVector(E_final[:, :2])
